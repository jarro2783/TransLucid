...
void GeneralContainer::addExpr(const Tuple& k, HD *e) {
}

TaggedValue GeneralContainer::operator()(const Tuple& k) {
}

void GeneralContainer::push_back(HD *v, int time) {
   mpq_class next = m_storage.size() == 0 ? mpq_class(0) : m_storage.rbegin()->first + 1;
...
void GeneralContainer::push_front(HD *v, int time) {
   mpq_class previous = m_storage.size() == 0 ? mpq_class(0) : m_storage.begin()->first - 1;
...
void GeneralContainer::replace_at(HD *v, const mpq_class& pos, int time) {
...
void GeneralContainer::insert_before(HD *v, const mpq_class& pos, int time) {
...
void GeneralContainer::insert_after(HD *v, const mpq_class& pos, int time) {
...
void GeneralContainer::remove(const mpq_class& start, const mpq_class& end, int time) {
