...
Range Range::parse(const ustring_t& text, const Tuple& context, Interpreter& i) {
   return Range();
}
...
void Range::print(std::ostream& os, const Tuple& context) const {
   if (m_lower == 0) {
      os << "inf";
   } else {
      os << *m_lower;
...
bool Range::within(const Intmp& value) const {
...
bool Range::within(const Range& other) const {
