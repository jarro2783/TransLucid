...
namespace
{
  ...
  //k dagger c
  void
  perturb(tuple_t& k, const Tuple& c)
  {
...
ValueContext
Evaluator::evaluate(AST::Expr* e, const Tuple& context)
{
...
size_t
Evaluator::lookupDim(const TypedValue& v)
{
  size_t i;
...
Data*
Evaluator::visitBinaryOpExpr(BinaryOpExpr* e, Data* data)
{
  ...
  switch (e->op.assoc)
  {
    ...
    case Parser::ASSOC_COMPARISON:
    {
      ...
      bool evalTrue = true;
      std::auto_ptr<ValueV>
        lhs(makeValue(e->operands.at(0)->visit(this, data)));
...
Data*
Evaluator::visitBooleanExpr(BooleanExpr* e, Data* data)
{
  return new ValueV(TypedValue(Boolean(e->value), m_registry.indexBoolean()),
                    makeContext(data)->c);
}

Data*
Evaluator::visitConstantExpr(ConstantExpr* e, Data* d)
{
...
Data*
Evaluator::visitBuildTupleExpr(BuildTupleExpr* e, Data* d)
{
  ...
  BOOST_FOREACH(Expr* expr, e->values)
  {
    ...
    tuple1.insert(std::make_pair(lhs, result->rhs));
    delete result;
  }

  return new ValueV(TypedValue(Tuple(tuple1), m_registry.indexTuple()), c->c);
}

Data*
Evaluator::visitConvertExpr(SpecialOpsExpr* e, Data* d)
{
  ...
  else
  {
    ConvertFunction convert =
      m_registry.findConverter(type->index(), value->value.index());
    return new ValueV(convert(value->value, c->c), c->c);
  }
}

Data*
Evaluator::visitDimensionExpr(DimensionExpr* e, Data* d)
{
...
Data*
Evaluator::visitHashExpr(HashExpr* e, Data* d)
{
  ...
  else
  {
    ...
    {
      delete v;
      return new ValueV(TypedValue(Special(Special::DIMENSION),
                        m_registry.indexSpecial()), c->c);
    }
  }
}

Data*
Evaluator::visitIdentExpr(IdentExpr* e, Data* d)
{
  ContextV* c = makeContext(d);

  std::pair<bool, TypedValue> cached =
    m_interpreter.warehouse().lookupCalc(e->id, c->c);
...
Data*
Evaluator::visitIfExpr(IfExpr* e, Data* d)
{
  ContextV* c = makeContext(d);
...
Data*
Evaluator::visitIntegerExpr(IntegerExpr* e, Data* d)
{
  ContextV* c = makeContext(d);
  return new ValueV(TypedValue(Intmp(e->m_value),
                               m_registry.indexIntmp()), c->c);
}

Data*
Evaluator::visitIsSpecialExpr(SpecialOpsExpr* e, Data* d)
{
  ContextV* c = makeContext(d);
  ValueV* v = makeValue(e->e->visit(this, d));
...
Data*
Evaluator::visitIsTypeExpr(SpecialOpsExpr* e, Data* d)
{
...
Data*
Evaluator::visitOpExpr(AST::OpExpr*, AST::Data*)
{
  return 0;
}

Data*
Evaluator::visitPairExpr(PairExpr* e, Data* d)
{
...
Data*
Evaluator::visitRangeExpr(RangeExpr* e, Data* d)
{
  ContextV* c = makeContext(d);
  ...
  return new ValueV(TypedValue(
    Range(lhs.get() != 0 ? &lhs->value.value<Intmp>().value() : 0,
          rhs.get() != 0 ? &rhs->value.value<Intmp>().value() : 0),
          m_registry.indexRange()), c->c);
}

Data*
Evaluator::visitUnaryExpr(UnaryExpr* e, Data* d)
{
