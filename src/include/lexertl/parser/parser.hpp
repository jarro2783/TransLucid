...
private:
    struct stoken_shim
    {
        ...
        stoken_shim (const stoken_shim &rhs_)
        {
            if (this != &rhs_)
            {
                _token = rhs_._token;
                const_cast<stoken_shim &>(rhs_)._token = 0;
            }
        }
        ...
        bool operator == (const string_token &rhs_) const
        {
            return *_token == rhs_;
        }
    };
    ...
    typedef std::vector<stoken_shim> string_token_vector;
        ...
        if (first_msb_ == second_msb_)
        {
                ...
                i_ < size_; ++i_)
            {
                insert_range (string_token (first_msb_, first_msb_),
                    *data2_[0][i_]._token, *data2_[1][i_]._token, data_);
            }
        }
        else
        {
        ...
        string_token_vector data_[2])
    {
        typename string_token_vector::const_iterator iter_ =
            std::find (data_[0].begin (), data_[0].end (), token_);

        if (iter_ == data_[0].end ())
        {
            data_[0].push_back (stoken_shim ());
            data_[0].back ()._token = new string_token (token_);
            data_[1].push_back (stoken_shim ());
            data_[1].back ()._token = new string_token (token2_);
        }
        else
        {
            const std::size_t index_ = iter_ - data_[0].begin ();

            data_[1][index_]._token->insert (token2_);
        }
    }
        ...
        const string_token &token3_, string_token_vector data_[3])
    {
        ...
        do
        {
            iter_ = std::find (iter_, end_, token_);
            ...
            if (iter_ == end_)
            {
                ...
                data_[1].push_back (stoken_shim ());
                data_[1].back ()._token = new string_token (token2_);
                data_[2].push_back (stoken_shim ());
                data_[2].back ()._token = new string_token (token3_);
                finished_ = true;
            }
            else
            {
                ...
                {
                    data_[2][index_]._token->insert (token3_);
                    finished_ = true;
                }
                else
                {
    ...
    void push_ranges (string_token_vector data_[2], const false_ &)
    {
        ...
        push_range (*viter_++->_token);
        push_range (*viter2_++->_token);
        sequence ();
    ...
    {
        typename string_token_vector::const_iterator viter_ =
            data_[0].begin ();
        typename string_token_vector::const_iterator vend_ = data_[0].end ();
        typename string_token_vector::const_iterator viter2_ =
            data_[1].begin ();
