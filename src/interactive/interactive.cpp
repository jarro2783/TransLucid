...
namespace TLInteractive {
   ...
   namespace {
      ...
      class StreamIterator : public Parser::IteratorTraits {
         ...
         result_type operator()() const {
            ...
            if (mbstowcs(&w, buf, 1) == size_t(-1)) {
               throw TL::ParseError("invalid byte senquence in input converting character");
            }
            return w;
         }
      ...
      class ParserIterator : public Parser::IteratorBase {
         private:
         typedef Spirit::multi_pass<StreamIterator,
         ...
         const wchar_t& dereference() const {
            return **m_iter;
         }
         ...
         bool equal(const IteratorBase& rhs) const {
            const ParserIterator *r = dynamic_cast<const ParserIterator*>(&rhs);
            if (r) {
               if (m_iter && r->m_iter) {
...
void System::postInputSignal(std::vector<TL::AST::Expr*> const& ev) {
...
void System::parseHeader(const std::string& file) {
   TL::Parser::HeaderGrammar headerParser(m_parseInfo, m_parsers);

   TL::ustring_t contents = Glib::locale_to_utf8(Glib::file_get_contents(file));
